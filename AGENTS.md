...
- PDF 性能关键路径：不要在滚动或连续缩放时对所有 stroke 重新跑 tldraw freehand；优先复用 `strokeRenderCache`、页级 render signature 和延迟高清重绘。
- PDF 快速连笔关键路径：不要只依赖 canvas 本身的 `pointerup`；保留 window 级 pointer end、`lostpointercapture` 和 stale pointer 恢复，避免 iPad Pencil 快速书写时下一笔被 `pointerId` 锁吞掉。

## BRAT 发布准备

`.github/workflows/release.yml` 会在推送版本标签时构建并上传 BRAT 需要的 release assets。优先使用与 `manifest.json` 版本一致的标签，例如 `0.2.12`。
