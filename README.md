...
- PDF 性能优化：按原始 PDF 页坐标缓存 tldraw stroke 几何和 Path2D，滚动时只保留视口附近页面的 ink canvas，擦除/框选/撤销只刷新受影响页面，缩放停止后再按像素预算重绘。
- PDF 缩放优化：committed canvas 在缩放期间保留旧 bitmap 跟随 CSS 缩放，重建时先在离屏 buffer 完成再提交到可见 canvas，避免笔迹先消失再出现。
- PDF 粗笔优化：实时书写使用轻量 live stroke 渲染，抬笔后再生成高质量 tldraw/freehand 笔迹，减少粗笔和高亮笔移动时的卡顿。
...
- iPad Pencil 快速落笔：兼容 WebKit 复用同一个 `pointerId` 且上一笔 `pointerup` 延迟/丢失的情况，新落笔会先收尾上一笔再立即开始当前笔。
