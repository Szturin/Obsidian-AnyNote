...
- 中文命令：常用命令已改为中文显示。
- PDF 手写批注：在 Obsidian 原生 PDF 视图上直接挂载 Canvas 标注层，不再打开新窗口或 iframe。
...
PDF 批注目前是覆盖层批注，工具栏直接浮在 Obsidian 的原生 PDF 视图内。它不会修改原 PDF 文件，也不会把批注编译回 PDF。
...
src/pdf/pdf-ink-modal.ts             PDF 原生视图 Canvas 覆盖层批注实现
