...
- PDF 自动入口：打开 PDF 时默认自动显示 AnyNote 手写工具栏，也可用左侧按钮、命令或右键菜单打开。
- PDF 工具：普通笔、原子笔、荧光笔、橡皮擦、框选、删除选中、撤销、重做、保存。
- PDF 导出：可生成新的 `*.anynote.pdf`，将当前手写 stroke 作为矢量批注写入 PDF。
- PDF 手感优化：使用 Pointer Events、`pointerrawupdate`、`getCoalescedEvents()`、压力值、自适应平滑、速度/压力混合线宽、短距预测层和 desynchronized canvas context 降低输入延迟。
...
打开 PDF 时默认会自动进入批注界面。PDF 批注窗口内还有 `导出带批注 PDF` 按钮；如不想自动弹出，可在设置中关闭 `打开 PDF 时自动进入批注`。
