...
- PDF 性能优化：按原始 PDF 页坐标缓存 tldraw stroke 几何和 Path2D，滚动时使用页级签名跳过无变化重绘，缩放时先复用现有 bitmap 跟随 CSS 缩放，停止后再高清重绘。
- PDF 快速连笔：补齐 `lostpointercapture` 和 window 级 `pointerup/pointercancel` 兜底，下一笔落下时可恢复被 iPad WebView 卡住的旧 pointer，并把抬笔后的全量渲染改为 RAF 调度，避免吞掉下一笔。
