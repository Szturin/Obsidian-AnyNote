...
- PDF 快速连笔使用 window 级 pointer end、`lostpointercapture` 和 stale pointer 恢复；抬笔提交只做轻量状态更新，主层重绘交给 RAF，减少下一笔落笔被阻塞。
...
已加入 `.github/workflows/release.yml`。推送与 `manifest.json` 版本一致的标签（例如 `0.2.12`）会生成 BRAT 需要的 release assets：`main.js`、`manifest.json`、`styles.css`、`versions.json`。
