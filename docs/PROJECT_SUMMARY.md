...
Obsidian AnyNote 是一个本地学习实验工程，用来研究 Obsidian 内的手写、绘图和 PDF 批注体验。当前代码集成了 `obsidian_ink` 的功能形态，并增加了中文命令和 PDF 原生视图覆盖层批注。
...
- tldraw 驱动的手写/绘图编辑体验。
- 中文命令入口。
- PDF 原生视图内直接叠加 Canvas 标注层和浮动工具栏，不再使用 modal、iframe 或独立窗口。
- 打开 PDF 时默认自动显示 AnyNote 批注工具栏；也支持左侧按钮、命令和右键菜单入口。
- PDF 工具栏包含“手/浏览”模式和当前工具选中状态。
...
- 参考 tldraw/freehand 的笔迹模型，把原始采样点转为带半径的 stroke points，再渲染为平滑轮廓填充。
...
- PDF 还不是逐页语义模型，当前覆盖层按原生 PDF view/stage 尺寸保存。
...
已加入 `.github/workflows/release.yml`。许可问题解决后，推送与 `manifest.json` 版本一致的标签（例如 `0.2.5`）会生成 BRAT 需要的 release assets：`main.js`、`manifest.json`、`styles.css`、`versions.json`。
