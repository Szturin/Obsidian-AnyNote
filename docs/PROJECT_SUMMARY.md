...
- PDF 工具栏包含“手/浏览”模式和当前工具选中状态；移动端/iPad 使用固定在屏幕底部的触控工具栏。
...
已加入 `.github/workflows/release.yml`。许可问题解决后，推送与 `manifest.json` 版本一致的标签（例如 `0.2.8`）会生成 BRAT 需要的 release assets：`main.js`、`manifest.json`、`styles.css`、`versions.json`。
