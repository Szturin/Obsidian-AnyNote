...
- 每个可见 PDF page 都拥有独立 committed/live/prediction canvas，滚动浏览时可以连续看到每页已有笔迹。
...
- PDF 重绘性能使用多层保护：stroke 几何/Path2D 缓存、页级 render signature 跳过、视口附近页虚拟化、canvas 像素预算限制、连续缩放延迟重绘。
- PDF 缩放时 committed canvas 不立即改尺寸清空，而是保留旧 bitmap 跟随 CSS 缩放；新尺寸页面先离屏绘制完成，再一次性提交到可见 canvas。
- PDF 当前笔画使用轻量 live stroke 实时渲染，抬笔后再进入高质量 tldraw/freehand committed 渲染，降低粗笔实时书写成本。
- PDF 快速连笔使用 window 级 pointer end、`lostpointercapture` 和 stale pointer 恢复；抬笔提交优先增量写入当前页 committed canvas，避免下一笔落笔被全量重绘阻塞。
- PDF 批注层已从单个移动 stage 重构为 per-page ink surface：每个可见 PDF page 有自己的 committed/live/prediction canvas、render signature 和 dirty-page 调度队列，落笔按触点命中真实 PDF page。
...
已加入 `.github/workflows/release.yml`。推送与 `manifest.json` 版本一致的标签（例如 `0.2.15`）会生成 BRAT 需要的 release assets：`main.js`、`manifest.json`、`styles.css`、`versions.json`。
